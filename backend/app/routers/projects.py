...
DEFAULT_PROJECT_ANALYSIS_PROMPT = """你是一个项目管理效率顾问。今天是 {today}。以下是当前所有活跃项目的详细信息，项目按层级结构组织（父项目→子项目）。父项目是组织容器，实际工作在子项目中推进。

进展记录仅包含近 {analysis_days} 天内的更新。请以最新的进展记录为准进行判断，不要基于已被后续更新推翻的旧信息得出结论。
...
def _build_project_analysis_prompt(db: Session):
    ...
    now = datetime.utcnow()
    cutoff = now - timedelta(days=ANALYSIS_RECENT_DAYS)
    ...
    def _format_project_section(p, indent="", is_child=False):
        ...
        # Recent updates — time-filtered to last N days, with count cap
        limit = 10 if is_child else 20
        sorted_updates = sorted(p.updates, key=lambda x: x.created_at or datetime.min, reverse=True)
        recent_updates = [u for u in sorted_updates if u.created_at and u.created_at >= cutoff]
        if not recent_updates and sorted_updates:
            recent_updates = sorted_updates[:3]
        sorted_updates = recent_updates[:limit]
        ...
        if children:
            ...
            recent_child = [(n, u) for n, u in all_child_updates if u.created_at and u.created_at >= cutoff]
            if not recent_child and all_child_updates:
                recent_child = all_child_updates[:5]
            for child_name, u in recent_child[:30]:
    ...
    today_str = now.replace(tzinfo=ZoneInfo("UTC")).astimezone(tz_shanghai).strftime("%Y-%m-%d")
    prompt = (template
              .replace("{projects_text}", projects_text)
              .replace("{today}", today_str)
              .replace("{analysis_days}", str(ANALYSIS_RECENT_DAYS)))
